...
import { buildSbtItemDataCell, buildSingleSbtDataCell, Queries, 
    SbtItemData, SbtItemDataBase, SbtSingleData } from "./Sbt.data"
...
export class Sbt implements Contract {
    ...
    async sendTransferEditorship(provider: ContractProvider, via: Sender, 
        params: {queryId?: number, newEditor: Address, responseTo: Address | null,
    ...
    async sendRevoke(provider: ContractProvider, via: Sender,
        params?: {queryId?: number}) {
    ...
    async sendTakeExcess(provider: ContractProvider, via: Sender,
        params?: {queryId?: number}) {
