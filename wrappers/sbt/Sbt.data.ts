...
export function buildSbtItemDataCell(data: SbtItemData) {
  ...
  let contentCell = (typeof data.content === 'string') ? 
    encodeOffChainContent(data.content) : data.content;
...
export function buildSbtItemDeployMessage(conf: {
  queryId?: number, collectionAddress: Address, passAmount: bigint,
  itemIndex: number, itemOwnerAddress: Address, itemContent: string, ownerPubKey: bigint}) {
...
export const Qeries = {
  transfer: (params: {queryId?: number, newOwner: Address | null,
    responseTo?: Address, forwardAmount?: bigint}) => {
    ...
    proveOwnership: (params: {queryId?: number, to: Address, data: Cell,
      ...
      requestOwnerInfo: (params: {queryId: number, to: Address, data: Cell,
        withContent: boolean}) => {
          let msgBody = beginCell();
      ...
      ownershipProof: (params: {queryId?: number, id: number, owner: Address,
        data: Cell}, bounced?: boolean) => {
          let msgBody = beginCell();
      ...
      ownerInfo: (params: {queryId?: number, id: number, initiator: Address,
        owner: Address, data: Cell}, bounced?: boolean) => {
          let msgBody = beginCell();
      ...
      getStaicData: (params: {queryId?: number}) => {
        let msgBody = beginCell();
      ...
      transferEditorship: (params: {queryId?: number, newEditor: Address,
        responseTo: Address | null, forwardAmount?: bigint}) => {
          let msgBody = beginCell();
