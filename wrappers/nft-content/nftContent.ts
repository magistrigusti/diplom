...
export function encodeOffChainContent(content: string) {
  ...
  if (prefix !== OFF_CHAIN_CONTENT_PREFIX) {
    throw new Error(`Unknow content prefix: ${prefix.toString(16)}`);
  }
...
export function encodeOnchainContent(
  content: Map<string, string | Buffer> | {[key: string]: string | Buffer}
): Cell {
  const keyLen = 256;
...
let g_supportedKeys = addKeysHashesToTheMap(new Map(), [
  "uri", "name", "description", "image", "image_data", "symbol", 
